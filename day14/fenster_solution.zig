...
fn solve2(input: []const u8, _: Allocator, comptime extent: Point) !usize {
    var screen_buffer: [extent.x * extent.y]u32 = undefined;
    ...
    while (c.fenster_loop(&f) == 0) {
        ...
        for (0..@intCast(extent.y)) |y| {
            for (0..@intCast(extent.x)) |x| {
                screen_buffer[y * extent.x + x] = 0x0;
                for (robots.slice()) |r| {
                    if (r.p.x == x and r.p.y == y) {
                        screen_buffer[y * extent.x + x] = 0xffffff;
                        break;
                    }
                }
            }
        ...
        if (step and std.mem.indexOf(u32, &screen_buffer, &[_]u32{0xffffff} ** 10) != null) {
            std.debug.print("{d} \n", .{seconds});
